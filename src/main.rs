...
use network::{NetworkMessage, NetworkClient, TransactionInfo};
...
pub struct Node {
    ...
    // Add a mempool to store pending transactions
...
impl Node {
    pub fn new(data_dir: &str) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mempool = Arc::new(Mutex::new(HashMap::new())); // Initialize empty mempool
    ...
    pub fn create_block_template(&self, miner_address: &str) -> types::Block {
        let blockchain = self.blockchain.lock().unwrap();
        blockchain.create_block_template(miner_address)
    }
    ...
    pub fn get_pending_transactions(&self, limit: usize) -> Vec<Transaction> {
        ...
        mempool.values().cloned().take(limit).collect()
    }
...
async fn handle_network_message(
    ...
    match message {
        ...
        NetworkMessage::GetTemplate => {
            // Get the miner's address from the request or use a default
            let miner_address = client.peer_address.clone();
            
            let template = node.create_block_template(&miner_address);
        ...
        NetworkMessage::NewTransaction { transaction } => {
            ...
            // Instead of immediately creating a block, add to mempool
            let tx_hash = transaction.hash();
            {
                ...
                if !mempool.contains_key(&tx_hash) {
                    ...
                    info!("Transaction added to mempool. Total pending: {}", mempool.len());
                }
            }
        ...
        NetworkMessage::GetPendingTransactions { limit } => {
            let transactions = node.get_pending_transactions(limit);
            info!("Sending {} pending transactions", transactions.len());
            
            client.send_message(&NetworkMessage::PendingTransactions { 
                transactions 
            }).await?;
        },
        ...
        NetworkMessage::GetTransactionHistory { address } => {
            ...
            client.send_message(&NetworkMessage::TransactionHistory { 
                address: address.clone(), 
                transactions: transactions.clone() // Added .clone() here
            }).await?;
